...
R_SCRIPT = "./predict_bart_cli.R"  # same folder as this app
...
def call_bart_cli(rows: list[dict]) -> pd.DataFrame:
    """
    Calls the R CLI to get PredVol_M and FT_Prob.
    rows: list of dicts with keys PMVolM, PMDolM, FloatM, GapPct, ATR, MCapM, Catalyst
    Returns a DataFrame with PredVol_M and FT_Prob aligned to input length.
    """
    if not shutil.which("Rscript"):
        raise RuntimeError("Rscript not found on PATH.")
    if not shutil.which(R_SCRIPT) and not R_SCRIPT.startswith("./"):
        # allow relative path
        pass
    # Ensure numeric JSON (convert None to 0/NaN as needed)
    payload = []
    for r in rows:
        payload.append({
    ...
    try:
        ...
        raise RuntimeError("Failed to parse R JSON: " + str(e) + "\nRAW:\n" + p.stdout.decode("utf-8"))
    df = pd.DataFrame(out)
...
# ---------- Sidebar weights (keep your originals) ----------
...
with tab_add:
    ...
    if submitted and ticker:
        ...
        try:
            cli_df = call_bart_cli([{
                ...
                "ATR":    atr_usd,
                "MCapM":  mc_m,
                "Catalyst": 1 if catalyst_points > 0 else 0,  # simple binarization
            }])
        except Exception as e:
            st.error("Could not load BART models (.rds). Ensure R is installed, required R packages are available, and the four model files plus predict_bart_cli.R are in the app folder.\n\nDetails:\n" + str(e))
            st.stop()

        pred_vol_m = float(cli_df.loc[0, "PredVol_M"])
        ft_prob    = float(cli_df.loc[0, "FT_Prob"])

        # Confidence bands for display (rough, user-configurable sigma)
        ...
        # Combine with your modifiers (no qual block UI here to keep focus)
        combo_pct   = num_pct
        ...
        st.session_state.flash = f"Saved {ticker} – {ft_label} ({ft_pct_str}) · Odds {row['Odds']} (Score {row['FinalScore']})"
        do_rerun()
    ...
    if l:
        ...
        # FT label + percentage shown neatly
        ...
        d4.metric("RVOL Weighted",     f"{st.session_state.get('w_rvol', w_rvol):.2f}")
...
with tab_rank:
    ...
    if st.session_state.rows:
        ...
        st.info("No rows yet. Add a stock in the **Add Stock** tab.")        return dict(
            ok=True, err=None,
