...
ss.setdefault("rows", [])  # LIST — preserved
...
# Winsorization & percent-normalization helpers
EPS = 1e-9
...
if build_btn:
    if not uploaded:
        ...
        try:
            ...
            # Remove risky double-scaling block (PM_Vol_% not used)
            # for pct_col in ("Gap_%","PM_Vol_%","Max_Pull_PM_%"): ...

            # Max_Push_Daily_% (fraction or % → normalize once)
            ...
            # Normalize percents ONCE
            for c in ("Gap_%","Max_Pull_PM_%","PM$Vol/MC_%","Max_Push_Daily_%"):
                if c in df.columns:
                    df[c] = _ensure_percent_once(df[c])
...
if submitted and ticker:
    ...
    # Winsorize same as base (single-point safe)
    def _winsor_one(x):
    ...
    try:
        if eval_ok:
            ...
            if np.unique(p_raw).size >= 3 and _has_both_classes(yva):
                ...
                if len(bx) >= 2: iso_bx, iso_by = np.array(bx), np.array(by)
        # More reliable fallback using TRAIN distribution if isotonic too coarse
        if iso_bx.size < 2:
...
def _delete_selected():
    ...
    if tickers_to_delete:
        ...
        st.session_state["align_sel_tickers"] = []
        # Immediate refresh for options + charts
...
# Lock features to avoid drift across thresholds
...
with st.spinner("Calculating distributions across all cutoffs..."):
    for thr_val in gain_cutoffs:
        ...
        if (vc.get(gA, 0) < 10) or (vc.get(gB, 0) < 10):
            continue
        # Extra guard: avoid extreme class imbalance (stabilizes calibration)
        ...
        # LOCK to your var_all (no per-threshold drift)
        req_feats = var_all[:]
        if added_df.empty: continue

        # Robust reindex to avoid missing cols crash; numeric+mask as you had
...
# --- Conditional Probability Chart Section (your math; honest labeling) ---
st.markdown("---")
...
for s in series_list:
    ...
    if len(x_fit) > 4:
        coeffs = np.polyfit(x_fit, y_fit, 4)  # keep your smoother
...
if cond_data:
    ...
    cond_chart = (
        ...
        .encode(
            ...
            y=alt.Y("Value:Q", title="Transition Ratio (%)", scale=alt.Scale(domain=[0, 100])),
        ...
        .properties(height=400, title=f"Transition Ratio {chart_title_suffix}")
    )
