...
# --- Controls row: multiselect, Clear, Delete, and split-mode dropdown ---
...
gain_cutoffs = list(range(25, 401, 25))  # <<< changed (25 -> 400)
...
def _make_split(df_base: pd.DataFrame, thr_val: float, mode: str):
    ...
    if mode == "Efficiency η":
        ...
        df_tmp["__Group__"] = np.where(m_eta >= 1.0, gA_, gB_)
        return df_tmp, gA_, gB_
...
MIN_PER_GROUP = 6  # <<< changed (was 10)
...
with st.spinner("Calculating distributions across all cutoffs..."):
    for thr_val in gain_cutoffs:
        ...
        # enforce counts after all filters for the mode
        vc = df_split["__Group__"].value_counts()
        if (vc.get(gA, 0) < MIN_PER_GROUP) or (vc.get(gB, 0) < MIN_PER_GROUP):
            continue
